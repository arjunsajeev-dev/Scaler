...
```bash
docker compose build demo alpha beta
```
...
```bash
docker compose build demo alpha beta
docker compose up -d redis traefik docker-socket-proxy
```
...
```
app/            FastAPI app, control loop, docker-py wrapper, Redis store, MQTT client
config/         per-service policy + container template
demo/           CPU-burnable image the orchestrator scales
```
